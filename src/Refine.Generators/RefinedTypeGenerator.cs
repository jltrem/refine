...
public class RefinedTypeGenerator : IIncrementalGenerator
{
                ...
                if (SymbolEqualityComparer.Default.Equals(attributeSymbol.ContainingType, expectedAttributeSymbol))
                {
                    ...
                    if (typeExpression is TypeOfExpressionSyntax typeofExpression)
                    {
                        ...
                        return new WrapperTypeInfo
                        {
                            ...
                            TargetIsValueType = targetTypeSymbol.IsValueType
                        };
                    }
                }
            }
    ...
    private static string GenerateWrapperSource(WrapperTypeInfo info)
    {
        ...
        List<string> possibleOtherMethods =
        [
            validateMethod,
            tryCreateMethod,
            MakeToStringMethod(info),
            MakeEqualsMethods(info, targetType),
            MakeComparableMethods(info, targetType),
            MakeEqualityOperators(info, targetType),
            MakeComparisonOperators(info, targetType),
            MakeCastOperators(info, targetType)
        ];

        StringBuilder otherMethods = new();
        foreach (var method in possibleOtherMethods.Where(method => !string.IsNullOrWhiteSpace(method)))
        {
            otherMethods.AppendLine();
            otherMethods.AppendLine(method);
        }
        
        var interfaces = MakeInterfaces(info, targetType);
        ...
        return $$"""
                 ...
                 {
                     public partial class {{info.ClassName}} {{interfaces}}
                     {
    ...
    private static string MakeInterfaces(WrapperTypeInfo info, string targetType)
    {
        var interfaces = MakeComparableInterfaces(info, targetType);

        var result = string.Join(", ", interfaces.Where(x => !string.IsNullOrWhiteSpace(x)));

        return result.Length > 0 ? ": " + result : "";
    }
    
    private static string[] MakeComparableInterfaces(WrapperTypeInfo info, string targetType)
    {
        bool comparable = info.Options.IsSet(MethodOptions.Comparable) && info.TargetHasComparable;
        bool comparableT = info.Options.IsSet(MethodOptions.Comparable) && info.TargetHasComparableT;
        bool equatableT = info.Options.IsSet(MethodOptions.Equatable) && info.TargetHasEquatableT;

        List<string> values = new();

        if (comparable)
        {
            values.Add("IComparable");
        }

        if (comparableT)
        {
            values.Add($"IComparable<{targetType}>");
            values.Add($"IComparable<{info.ClassName}>");
        }

        if (equatableT)
        {
            values.Add($"IEquatable<{targetType}>");
            values.Add($"IEquatable<{info.ClassName}>");
        }
        
        return values.ToArray();
    }
    
    private static string MakeComparableMethods(WrapperTypeInfo info, string targetType)
    {
        bool comparable = info.Options.IsSet(MethodOptions.Comparable) && info.TargetHasComparable;
        bool comparableT = info.Options.IsSet(MethodOptions.Comparable) && info.TargetHasComparableT;
        bool equatableT = info.Options.IsSet(MethodOptions.Equatable) && info.TargetHasEquatableT;
        string nullable = info.TargetIsValueType ? "" : "?";

        StringBuilder sb = new();

        if (comparable || comparableT)
        {
            sb.AppendLine("        // MethodOptions.Comparable");
        }

        if (comparable)
        {
            sb.AppendLine($"        public int CompareTo(object? obj) => obj is {targetType} ? Value.CompareTo(({targetType})obj) : 0;");
        }

        if (comparableT)
        {
            sb.AppendLine($"        public int CompareTo({targetType}{nullable} other) => Value.CompareTo(other);");
            sb.AppendLine($"        public int CompareTo({info.ClassName}? other) => (other is null) ? 0 : Value.CompareTo(other.Value);");
        }

        if (equatableT)
        {
            sb.AppendLine("        // MethodOptions.Equatable");
            sb.AppendLine($"        public bool Equals({targetType}{nullable} other) => Value.Equals(other);");
            sb.AppendLine($"        public bool Equals({info.ClassName}? other) => (other is null) ? false : Value.Equals(other.Value);");
        }
...
internal record WrapperTypeInfo
{
    ...
    public bool TargetIsValueType { get; set; }
}
